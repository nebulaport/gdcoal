    ...
    <script type="text/javascript">
        ...
        function addDetailRow(){
           ...
           /* var items= $("#coalRow tr").get();
            var tr="<form   enctype='multipart/form-data'    method='post'  ><tr ><td></td>"
                    +" <td class='text-nowrap' scope='row'>"+(items.length+1)+"</td>"
                    +"<td>"
                    +"  <input   type='text'  readonly class='form-control'  value='"+d+"' name='supplementid'>"
                    +" </td>"
                    +"<td>"
                    +" <input   type='text' required class='form-control'    name='supplementdesc' >"
                    +" <span></span></td>"
                    +"<td>"
                    +" <input   type='text' readonly  class='form-control' value='"+getNowFormatDate()+"'   name='uploaddate'>"
                    +"<span></span> </td>" +
                    "<td><input type='file' class='form-control' name='supportFile'></td>"
                    +" <td>"
                    +" <a   class='btn btn-primary'  href='javascript:void(0)' onclick='deleteRow(this)'>删除</a>"
                    +" <input type='submit'   class='btn btn-primary'  value='上传' ></td>"
                    +"<td></td></tr></form>";
            $("#coalRow").append($(tr));*/
            $("#supplementid").val(d);
            $("#uploaddate").val(getNowFormatDate());
    ...
    <div class="panel panel-default">
        <div class="panel-heading" style="background-color:#05298F">
            <h2 class="panel-title" style="font-weight:bold; color:#fff;">
                长协交易管理-<span style="font-size:14px;">添加长协补充协议</span>
            </h2>
        </div>
            ...
            <div class="formRow">
                <div class="form-group">
                    ...
                    <c:if test="${entity.contractfilename!=null}">
                        <a href="<%=basePath%>/bondCtractManager/downLoadBondContractFile?contractNo=${entity.contractNo}">下载</a>
                    </c:if>
                </div>
            </div>
                    ...
                    <div class="panel-body">
                        <table class="table table-hover font-table">
                            ...
                            <tr>
                                ...
                                <th><a class="btn btn-primary" href="javascript:void(0)"  data-toggle="modal" data-target="#myModal" onclick="addDetailRow()">添加</a></th>
                                <th></th>
                            </tr>
                            </thead>
                            <tbody id="coalRow">
                            <c:forEach items="${supportList}" var="e" varStatus="a">
                            <tr>
                                 <td>
                                 </td>
                                <td class='text-nowrap' scope='row'>
                                ...
                                <td>
                                    <input   type='text'  readonly class='form-control'  value='${e.supplementid}' name='supplementid'>
                                    </td>
                                <td>
                                     <input   type='text' readonly  class='form-control'   value='${e.supplementdesc}'   name='supplementdesc' >
                                    <span></span></td>
                                <td>
                                     <input   type='text' readonly  class='form-control' value='${e.uploaddate}'   name='uploaddate'>
                                    <span></span> </td>
                                <td>${e.supplementfilename}

                                </td>
                                <td>
                                    <c:if test="${e.supplementfilename!=null}">
                                    <a class='btn btn-primary' href="<%=basePath%>/bondCtractManager/downLoadBondContractSupportFile?supplementid=${e.supplementid}">下载</a>
                                    </c:if>
                                     <a   class='btn btn-primary'  href='<%=basePath%>bondCtractManager/deleteCtractSupport?supplementid=${e.supplementid}&contractNo=${entity.contractNo}' >删除</a>
                                <td>
                                </td></tr>
                                </c:forEach>
            ...
            <div class="formRow" style="text-align: center;">
                <div class="form-group" >
                    <c:if test="${entity.status==1}">
                        <a  href="<%=basePath%>bondCtractManager/bondCtractSupportSubmit?contractNo=${entity.contractNo}" id="submitBtn"  class="btn btn-primary" >提交</a>
                    </c:if>
        ...
        <div class="modal-content">
            <div class="modal-header">
              ...
              <div class="modal-body" style="height:400px;width:600px;">
                  <div class="panel-body">
                      <div class="formRow">
                          <form   class="form-inline" enctype='multipart/form-data' action='<%=basePath%>bondCtractManager/addBondctractSupportRow?contractNo=${entity.contractNo}' method='post'  >
                          <div class="form-group">
                              <label for="supplementid">协议编号</label>
                              <input  class="form-control"  type='text'  readonly id="supplementid"   name='supplementid'>
                           </div>
                          <div class="form-group">
                              <label for="supplementdesc">摘要</label>
                              <input   class="form-control"  type='text' required      id="supplementdesc" name='supplementdesc' >
                          </div>
                          <div class="form-group">
                              <label for="uploaddate">上传时间</label>
                              <input  class="form-control"  type='text' readonly   id="uploaddate"  name='uploaddate'>
                           </div>
                          <div class="form-group">
                              <label for="uploaddate">文件</label>
                               <input class="form-control" type='file'  name='supportFile'>
                           </div>
                          <div class="form-group">
